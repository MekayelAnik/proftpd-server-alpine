...
if [[ "${NUMBER_OF_SHARES}" =~ ^[0-9]+$ ]] || [[ "${NUMBER_OF_SHARES}" -le 0 ]] || [[ "${NUMBER_OF_SHARES}" != '' ]]; then
	if [[ "${NUMBER_OF_SHARES}" -ge 1 ]] && [[ "${NUMBER_OF_SHARES}" -le 2147482647 ]]; then
		...
		echo "The NUMBER_OF_SHARES is set to ${NUMBER_OF_SHARES}, which is out of optimal range. Set value between 1000 to 2147483647 in NUMBER_OF_SHARES. Otherwise Exitting..."
		exit 1
	fi
else
	echo "Set a integer Greater than 0 and less than 2147482647 value in NUMBER_OF_SHARES. Otherwise No Share will be created. Exitting..."
	exit 1
fi
